...
extern int sockfd, cliente_sockfd;
...
void cliente(int cliente_sockfd)
{
    ...
    struct datosf temp;
    char latV[10]="N o S: ", lonV[10]="E o W: ", time[20]="Tiempo: ", lat[20]="Latitud: ", lon[20]="Longitud: ";
    ...
    //printf(">>>>>%f %f %c %f %c \n", temp.time,temp.lat,temp.latV, temp.lon, temp.lonV);
	//gcvt(temp.time,12,time);
    printf ("------%s", time);
	if (write(cliente_sockfd, temp.time, 20) < 0)
	{
		perror("Ocurrio un problema en el envio de un mensaje al cliente");
		exit(EXIT_FAILURE);
	}
    if (write(cliente_sockfd, &temp, sizeof(temp)) < 0)
	{
		perror("Ocurrio un problema en el envio de un mensaje al cliente");
		exit(EXIT_FAILURE);
	}
    // else
    // {
    //     printf("Se envio correctamente la informcaion\n");
    // }
